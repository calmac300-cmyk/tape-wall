...
- **Deck:** A fixed bottom housing (`.deck-fascia`) styled as a real deck: a large recessed `.window` (up to 300px wide) holding the SVG cassette with reels that shrink and grow with side progress, auto-reverse from side A to side B; nameplate (title/artist/counter) below the window; transport buttons (previous, play/pause, next, flip, eject) grouped into a recessed `.controls` panel underneath, mounted-looking rather than free-floating. Lock-screen controls use the Media Session API.
